...
import type { AuthState, ComparisonResult, FinanceTask, HistoryEntry, ProviderOption } from "@/types";
...
function toHistoryEntry(response: ComparisonResult["left"], context?: string): HistoryEntry {
  return {
    ...response,
...
export function ComparePanel({
  ...
  const [prompt, setPrompt] = useState("Compare the risks and outlook in the following company excerpt.");
  const [context, setContext] = useState("Operating cash flow improved, but debt costs remain elevated and management signaled a cautious outlook.");
  ...
  async function handleCompare() {
    ...
    try {
      ...
      const leftEntry = toHistoryEntry(comparison.left, context.trim() || undefined);
      const rightEntry = toHistoryEntry(comparison.right, context.trim() || undefined);
  ...
  return (
    <section className="space-y-6 rounded-[32px] border border-white/10 bg-panel/85 p-6 shadow-halo backdrop-blur-xl">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <p className="text-xs uppercase tracking-[0.28em] text-cyan-200/70">Compare mode</p>
          <h2 className="mt-2 text-2xl font-semibold text-white">Run the same prompt against two providers</h2>
        </div>
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <label className="block space-y-2">
          <span className="text-sm font-medium text-slate-200">Left provider</span>
          <select
        ...
        <label className="block space-y-2">
          <span className="text-sm font-medium text-slate-200">Right provider</span>
          <select
      ...
      <div className="flex flex-wrap gap-3">
        {(["summarize", "qa", "risk-analysis"] as FinanceTask[]).map((item) => (
          <button
            key={item}
            type="button"
            onClick={() => onTaskChange(item)}
            className={`rounded-2xl border px-4 py-2 text-sm transition ${task === item ? "border-cyan-300/40 bg-cyan-300/10 text-white" : "border-white/10 bg-white/0 text-slate-300 hover:border-white/20 hover:bg-white/5"}`}
          >
            {item}
          </button>
        ))}
      ...
      {result ? (
        <div className="grid gap-5 xl:grid-cols-2">
          <ResponseCard response={result.left} accent="from-cyan-400/40 to-sky-500/20" />
          <ResponseCard response={result.right} accent="from-fuchsia-400/30 to-amber-400/20" />
        </div>
      ) : (
