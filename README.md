...
```bash
docker exec -it finance-ai-platform python src/evaluation/evaluate_model.py \
  --mock \
  --dataset data/instruction/finance_gold_v1.jsonl \
  --output-dir reports/evaluation \
  --run-name mock_eval_full \
  --overwrite
```
...
```text
reports/evaluation/mock_eval_full/predictions.jsonl
reports/evaluation/mock_eval_full/metrics.json
reports/evaluation/mock_eval_full/summary.md
```

### run a small balanced evaluation
...
### run fine-tuned adapter evaluation

```bash
docker exec -it finance-ai-platform python src/evaluation/evaluate_model.py \
  --dataset data/instruction/finance_gold_v1.jsonl \
  --output-dir reports/evaluation \
  --run-name qlora_finance_eval \
  --model-name Qwen/Qwen2.5-3B-Instruct \
  --adapter outputs/qlora-finance \
  --max-examples-per-task 1 \
  --max-new-tokens 80 \
  --overwrite
```
