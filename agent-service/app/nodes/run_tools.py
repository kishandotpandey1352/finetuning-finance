...
from app.tools.registry import run_financial_calculator, run_memory_lookup
...
def run_tools_node(state: FinanceAgentState) -> FinanceAgentState:
    ...
    if tool_plan.use_financial_calculator and tool_plan.financial_calculator_input:
        calculator_input = tool_plan.financial_calculator_input

        try:
            calculator_output = run_financial_calculator(calculator_input)
            tool_results.append(
                ToolCallRecord(
